...
def bce(pred, label):
    ...
    neg_cof = torch.sum(pos) / total * 1.1
...
if __name__ == '__main__':
    ...
    name = 'SE'
    dic = {'BG': 0, 'HE': 1, 'EX': 2, 'MA': 3, 'SE': 4}
    path = 'Data_Group_Component_Task_1/Train'

    image_dir = path + '/original_retinal_images'
    path_dic = dict()
    path_dic['HE'] = path + '/masks_Haemorrhages'
    path_dic['EX'] = path + '/masks_Hard_Exudates'
    path_dic['MA'] = path + '/masks_Microaneurysms'
    path_dic['SE'] = path + '/masks_Soft_Exudates'
    ...
    nb_of_class = 2
    ratio = 20
    height, width = int(2848 / ratio)+1, int(4288 / ratio)+1
    X_train = None
    y_train = None
    for i in range(1, nb_of_img+1):
        ...
        image = cv2.imread(image_dir + f'/IDRiD_{idx}.jpg', 1)
        ...
        b = clahe.apply(b).reshape(height, width, 1)
        g = clahe.apply(g).reshape(height, width, 1)
        r = clahe.apply(r).reshape(height, width, 1)
        ...
        flag = True
        for e in [name]:
            tmp = cv2.imread(path_dic[e] + f'/IDRiD_{idx}_{e}.tif', 0)
            if tmp is None:
                nb_of_img -= 1
                flag = False
                continue
    ...
    nb_of_test = 3
    test_path = 'Data_Group_Component_Task_1/Test'
    image_dir = test_path + '/original_retinal_images'
    path_dic = dict()
    path_dic['HE'] = test_path + '/masks_Haemorrhages'
    path_dic['EX'] = test_path + '/masks_Hard_Exudates'
    path_dic['MA'] = test_path + '/masks_Microaneurysms'
    path_dic['SE'] = test_path + '/masks_Soft_Exudates'
    for i in range(55, nb_of_test + 55):
        print(i)
        idx = str(i)
        if i < 10:
            idx = '0' + idx
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(int(X_train.shape[0] / batch_size)):
            ...
            if target.size()[2] - output.size()[2]:
                target = F.pad(target, (-1, 0, -1, 0))
        ...
        with torch.no_grad():
            ...
            if epoch % 1 == 0:
                plt.subplot(2, 2, 1)
                plt.imshow(target.numpy().astype(np.uint8)[0, 0, :, :] * 50)
            ...
            if epoch % 1 == 0:
                ...
                # print(y_hat.shape)
                # print(np.max(y_hat.numpy().astype(np.uint8)[2, :, :]), np.min(y_hat.numpy().astype(np.uint8)[2, :, :]))
                plt.imshow(y_hat.numpy().astype(np.uint8)[0, :, :] * 50)
